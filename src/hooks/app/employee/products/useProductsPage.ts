...
export const useProductsPage = () => {
    ...
    const {state: addProductDialogState, ...addProductEventSenders} = useCustomMachine(MachineType.ADD_PRODUCT);
    const addProductFormProps = useForm();

    const {state: updateStockDialogState, ...updateStockEventSenders} = useCustomMachine(MachineType.UPDATE_STOCK);
